...
ep = proj.fit_transform(e)

from collections import Counter
def cnt_hydrophobic(token):
    return round(Counter(token)['e'] / len(token), 4)
...
with open('projection_embedding.csv', 'w+') as out:
    out.write('token,hydrophobic,dist,c1,c2\n')
    for token, (c1, c2) in zip(TEXT.vocab.itos, ep):
        if levenshteinDistance('bcebbee', token) < 2:
            dist = 'close'
        else:
            dist = 'appart'

        out.write(f'{token},{cnt_hydrophobic(token)},{dist},{c1},{c2}\n')


'''
library(ggplot2)
library(readr)
df <- read_csv('projection_embedding.csv')
ggplot(df, aes(x=c1, y=c2, color=dist)) + geom_point(size=1) + theme_classic()

Neither fraction of hydrophobic AAs nor sequence identity (one token) seems to cluster.

Explore more -- https://projector.tensorflow.org/
'''


with open('embedding_vectors.tsv', 'w+') as out:
    for i in e:
        out.write('\t'.join([str(j) for j in i]) + '\n')

with open('embedding_labels.tsv', 'w+') as out:
    for i in TEXT.vocab.itos:
        out.write(i + '\n')


'''
Most signal seems to come from homologue tokens ...
'''
